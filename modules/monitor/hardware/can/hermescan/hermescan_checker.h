...
#ifndef MODULES_MONITOR_HARDWARE_CAN_HERMESCAN_HERMESCAN_CHECKER_H_
#define MODULES_MONITOR_HARDWARE_CAN_HERMESCAN_HERMESCAN_CHECKER_H_
...
class HermesCanChecker : public HwCheckerInterface {
 public:
  static const char HERMES_CAN_NAME[];

  /// Returns a HW status code from ntstatus.
  static HardwareStatus::Status hermescan_result_to_hw_status(
      int bcanstatus);

  /// Returns a HW status message from ntstatus.
  static std::string hermescan_result_to_message(int ntstatus);

  HermesCanChecker();

  virtual ~HermesCanChecker() {}
